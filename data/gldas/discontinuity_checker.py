...
if __name__ == '__main__':
    ...
    variables = {
        ...
        'soil_moisture_0-10cm': 'soilm',
    }

    alpha = 0.05

    model_change_years = [1999, 2000]
    ...
    for variable, sname in variables.items():
        ...
        with Dataset('%s/%s_%04d.nc'%(folder, filename, 1999), 'r') as nc:
            data1 = nc.variables[sname][:]

        with Dataset('%s/%s_%04d.nc'%(folder, filename, 2000), 'r') as nc:
        ...
        cbar.ax.set_xlabel('Strength of V2.1 Dominance over V2.0', fontsize = 22)
        ...
        offset = (datetime(1999,1,1) - datetime(1970,1,1)).days
        taus = taus + offset
        ...
        dates = np.array([datetime(1999,1,1) + timedelta(days = day) for day in range(cmax)])
