...
if __name__ == '__main__':
    ...
    # Base paths
    base_path = '/ourdisk/hpc/ai2es/sedris/era5'
    base_write = '/ourdisk/hpc/ai2es/sedris/fd_analysis/data'

    for m, variable in enumerate(variables):
        # Collect all the .nc files to be examined
        files = glob('%s/%s/%s*.nc'%(base_path, directories[m], variable), recursive = True)
        ...
        for n, file in enumerate(new_sort(files)):
            ...
            # Subset the data
            data_sub, lat_sub, lon_sub = subset_data(data, lat[:,0], lon[0,:], subset = 'africa')
